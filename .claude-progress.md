...
**Branch**: `main` (commit: `dab05ba` — Phase 3 complete)
**Status**: Phase 3 done. Process matching, ciphertext read, perm enforcement all tested.
...
sudo bash ~/ecryptfs/r.sh

# 2. Re-add ACL rules (in-memory only — lost on rmmod)
echo "add 1 100 * * /usr/bin/cat  r ciphertext" | sudo tee /sys/kernel/debug/ecryptfs_acl/0/control
echo "add 1 50  * * /usr/bin/head r plaintext"  | sudo tee /sys/kernel/debug/ecryptfs_acl/0/control

# 3. Verify
sudo cat /sys/kernel/debug/ecryptfs_acl/0/rules

# 4. Run automated test suite
sudo python3 -m pytest ~/ecryptfs/tests/ -v

# Or run a single file:
sudo python3 -m pytest ~/ecryptfs/tests/test_basic.py -v
```

> xattrs on lower files (`trusted.ecryptfs_acl_id`) persist — no need to re-set them.

### Resume with Claude
Start a new conversation and say: **"resume the ecryptfs ACL project"**
