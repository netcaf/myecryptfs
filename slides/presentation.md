...
- **Inheritance**: dynamic upward walk at access time — xattr present = explicit override, absent = inherit (unambiguous by construction)
...
```bash
add <acl_id> <priority> <uid> <gid> <proc> <perm> <content>
```
...
**Tag a file and add rules:**

```bash
setfattr -n trusted.ecryptfs_acl_id -v 0x0001 /testbase/data.enc/demo.txt
echo "add 1 100 * * /usr/bin/head r plaintext"  | sudo tee /sys/kernel/debug/ecryptfs_acl/0/control
echo "add 1  50 * * /usr/bin/cat  r ciphertext" | sudo tee /sys/kernel/debug/ecryptfs_acl/0/control
```

**Three processes, same file, three different outcomes:**

```bash
head /testbase/data.view/demo.txt   # → decrypted plaintext
cat  /testbase/data.view/demo.txt   # → raw ciphertext bytes
tail /testbase/data.view/demo.txt   # → EACCES  (no matching rule)
```
