...
### Design Rationale: Dynamic Walk vs. Eager Propagation
...
Copy the parent's ACL ID onto every new child at creation time. On retag, recursively update all descendants (analogous to `chmod -R`).

**Strengths:**
- xattr is always present on every file — ACL lookup is a direct O(1) xattr read, no walk needed
- No per-access dentry traversal cost
- Simple to reason about: the xattr on a file is always the authoritative, self-contained answer
- Consistent with how standard Unix permission changes work (`chmod -R`, `chown -R`)

**Weakness — ambiguous override semantics:**
After eager propagation has run, every child carries an xattr. Later, if an admin wants to set an independent rule on one specific file, they write a different ACL ID to it. But now two questions arise on retag:
- Which children were eagerly copied from the parent, and which were explicitly set by an admin?
- Should a recursive retag overwrite all children, or only the ones that were originally inherited?

There is no structural way to distinguish the two cases — both look identical (an xattr with a value). Any solution requires an extra mechanism: a "pinned" flag, a separate tracking file, or a convention. The design becomes more complex to implement and operate correctly.

---

#### Dynamic Walk (chosen)

A file with no xattr inherits by walking up the dentry tree at access time. A file with its own xattr uses that ACL and ignores all ancestors.

**Strengths:**
- xattr presence/absence has unambiguous meaning:
  - xattr **present** → explicitly tagged; independent ACL, not inherited
  - xattr **absent** → inherits; always reflects the current nearest ancestor
- Per-file override requires no extra mechanism — just set an xattr on the file
- Retag a directory: all untagged descendants automatically see the new ACL; explicitly tagged files are untouched by construction

**Weakness:**
- Dentry walk on every access to an uncached inode
- Requires hooking into no write paths (`create`/`mkdir` need no changes), but adds walk logic to the read/permission path

**Mitigation**: walk result is cached per-inode in `cached_acl_id` after first resolution, so the walk cost is paid only once per inode lifetime.

---

**Summary:**

| | Dynamic Walk | Eager Propagation |
|---|---|---|
| ACL lookup cost | Walk (once, then cached) | O(1) xattr read |
| Per-file override | Natural — just set xattr | Requires extra "pinned" flag |
| Retag semantics | Unambiguous | Ambiguous without extra tracking |
| Write path changes | None | Must hook `create` / `mkdir` |
| Consistency with Unix | Different | Matches `chmod -R` mental model |

Dynamic walk was chosen primarily for the unambiguous override semantics. The walk cost is negligible in practice due to per-inode caching.
